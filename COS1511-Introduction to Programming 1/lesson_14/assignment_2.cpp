...
void displayMenu(const vector<Flight>& flights) {
    ...
    cout << setw(25) << setfill('-') << "" << endl; // Divider
...
void displaySeating(const Flight& flight) {
    ...
    for (int i = 0; i < NUM_SEATS; ++i) {
        ...
        int seatNum = (i % 6) + 1;  // Seat numbers range from 1 to 6
        
        // Print seat label without extra spaces
        cout << " |  " << (flight.seats[i] ? "**" : rowLabel + to_string(seatNum)) << " ";

        // Add spacing after seat 3
        if (seatNum == 3) {
            cout << " | ------ ";
        }

        // Start a new line after every 6 seats (new row)
        if (seatNum == 6) {
            cout << " |\n";
            rowLabel++;  

            // Separate First Class from Economy Class
            if (rowLabel == 'E') {
                cout << " | Economy class (1600.00)\n";
            }
        }
    }
}
...
void displayBookingTicket(const string& fullName, const Flight& flight, int seatNumber) {
    ...
    // Determine if the seat is in First Class or Economy
    ...
    // Calculate arrival time (add 2 hours and 30 minutes)
    ...
    const int columnWidth1 = 20;
    const int columnWidth2 = 30;

    cout << left << setw(columnWidth1) << "Name:" << setw(columnWidth2) << fullName
        << "Travel Ticket class\t: " << seatClass << endl;

    cout << left << setw(columnWidth1) << "Departure:" << setw(columnWidth2) << "Johannesburg"
        << "Seat Number\t\t: " << seatNumber << endl;

    cout << left << setw(columnWidth1) << "Destination:" << setw(columnWidth2) << "Cape Town"
        << "Departure Time\t: " << flight.departureTime << endl;

    cout << left << setw(columnWidth1) << "" << setw(columnWidth2) << "" 
    ...
    cout << "Amount: R" << ticketPrice << " Thank you for booking with COS1511."
            << " Your travel agent for this querry is HM" << endl;
    cout << "*****************************\n";
    cout << "------------------------------------------------------------------------------\n";
}
