...
struct Flight {
    ...
    vector<bool> seats;
    int bookings;
    Flight(string time) : departureTime(time), seats(NUM_SEATS, false),
bookings(0) {}
};
// Function prototypes
...
void displayBookingTicket(const string& fullName, const Flight& flight, int
seatNumber);
void displayBookingSummary(const vector<Flight>& flights);
int main() {
    // Initialize flight times
    vector<Flight> flights;
    flights.push_back(Flight("7:00"));
    flights.push_back(Flight("9:00"));
...
cout << "*   Welcome to COS1511 Flight Booking System     *" << endl;
    ...
    do {
        displayMenu(flights);
        int choice;
   cout << "Choose the time by entering the option number from the displayed"
<< "list: (1-5): ";
        cin >> choice;
        ...
        cin >> continueBooking;
        cin.ignore();
    } while (toupper(continueBooking) == 'Y'); // Continue booking if the user
// enters 'Y' or 'y'
    displayBookingSummary(flights); // Display the total number of bookings for
// each flight time
...
void displayMenu(const vector<Flight>& flights) {
    ...
    cout << setw(5) << "Option" << setw(10) << "Depart" << setw(10) <<
"Arrive\n";
...
to_string(arrivalHour) + ":30";
        cout << setw(5) << to_string(i + 1) + "." << setw(10) <<
flights[i].departureTime << setw(10) << arrivalTime << endl;
    }
}
...
<< " asterisk";
        ...
        if (seatInput.length() != 2) {
            cout << "Invalid format! Please enter a seat number like A1, B2, "
<< " etc.\n";
continue; }
...
":30" << setfill(' ') << endl;
    ...
    cout.precision(2);
    cout << "*****************************\n";
    cout << "Amount: R" << ticketPrice << " Thank you for booking with"
    << " COS1511."
